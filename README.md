...
_The name comes from_ kasane no irome, _the Heian-era practice of layering
translucent garments so that the overlapping colors form a new composite hue —
which is, quite literally, what this program does._
...
この SVG 自体が下記 Layer 4 のキーフレーム書き出しそのもの
</sub></p>
...
README の画像はすべて Kasane 自身の出力で、次で再生成できる(シード固定なので決定的):
